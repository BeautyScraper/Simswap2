...
def dofsmage(srcfileps,targetfps,resultfp=""):
    ...
    with torch.no_grad():
        ...
        pic_a3 = r"D:\paradise\stuff\simswappg\srcs\ng\ChuchiDevi.jpg"
        pic_a2 = r"D:\paradise\stuff\simswappg\srcs\yummyx\Yummyx (16).jpg"
        pic_a4 = r"D:\paradise\stuff\simswappg\srcs\gudYummy\later\aYummyF.jpg"
        con_code = r"D:\paradise\stuff\simswappg\srcs\gudYummy\latent_db\concocted_mean.latent"
        latent_f = latent(app, transformer_Arcface,model,opt)
        # code_dir = Path(pic_a).parent / 'latent_records'
        get_code = latent.get_code
        latend_id = latent_f.get_latent(pic_a,get_code(pic_a)) 
        # latend_id = latent_f.get_mean_latent(r'D:\paradise\stuff\Essence\FS\all\Devi\ForImage - Copy') 
        # latend_id1 = (1.8*latend_id + 0.4 * latent_f.get_latent(pic_a2,get_code(pic_a2)) + 0.3 * latent_f.get_latent(pic_a3,get_code(pic_a3))+ 0.3 * latent_f.get_latent(pic_a4,get_code(pic_a4)))*(0.5)
        # latent.store_latent(latend_id, con_code)
        latend_id1 = latend_id
...
if __name__ == '__main__':
    ...
    dstvideodir = Path(r'D:\paradise\stuff\simswappg\Yummygrp')
    # dstvideodir = Path(r'D:\paradise\stuff\Essence\FS\all\Devi\ForImage - Copy (2)')
    # dstvideodir = Path(r'D:\paradise\stuff\simswappg\trialTargets')
    ...
    for vidFIle in dstFileList:
      ...
      for imgFiles in srcFileList:
        ...
        try:
            ...
            print(str(imgFiles), str(inputFilepath),str(rsultFilefp),e)
            print(str(imgFiles), str(inputFilepath),str(rsultFilefp)+e)
            # print()
            # import pdb;pdb.set_trace()
        # input Filepath.unlink()
