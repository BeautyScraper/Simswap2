...
def dofsmage(srcfileps,targetfps,resultfp="",face_intensity=0.5):
    ...
    with torch.no_grad():
        ...
        latend_id1 = latend_id + face_intensity * (latend_id)
        weight = face_intensity * 100
...
if __name__ == '__main__':
    ...
    for vidFIle in dstFileList:
      ...
      for imgFiles in srcFileList:
        for weight in np.arange(0,2.0, 0.1):
            # nextTargetfileName =  str (randint(1,1000)) + vidFIle.name
            nextTargetfileName =  vidFIle.name
            rsultFilefp = resultDir / nextTargetfileName
            dofsmage(str(imgFiles), str(inputFilepath),str(rsultFilefp),weight)
