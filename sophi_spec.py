...
def spicoutvid(inimgpath,vpath,rpath):
    ...
    vPath = Path(vpath)
    specific_img_dir = vPath.parent.parent / 'mc'
    specific_face_image_path = [x for x in specific_img_dir.glob('*.jpg') if vPath.stem in x.stem]
    if len(specific_face_image_path) == 0:
        for x in specific_img_dir.glob('*.jpg'):
            if x.stem.split('-Scene-')[0] in vPath.stem:
                pic_specific = str(x)
                break
        else:
            raise Exception('no specific face image found')
    else:
        pic_specific = str(specific_face_image_path[0])
    ...
    with torch.no_grad():
        ...
        video_swap(vpath, latend_id,specific_person_id_nonorm, opt.id_thres, \
